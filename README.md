...
- `time-domain` vector: The time intervals for which the model is applied. For example, in the `data_dropout` dataset, no events occur in the first semester, so the `time-domain` starts at the end of the first semester (t = 1) and ends at the end of the third year (t = 6).
