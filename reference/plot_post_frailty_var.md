...
This function plots the posterior frailty variances for each group in
...
variance or its time-independent and time-dependent components based on
...
\epsilon\_{jk}, \forall j,k\\ and the posterior frailty variance as
\\var(\hat{Z}\_{jk}) = var(\hat{\alpha}\_{j}/\hat{\alpha}\_{max}) +
var(\hat{\epsilon}\_{jk}/\hat{\epsilon}\_{max}\\), this function allows
plotting either the entire posterior frailty variance
